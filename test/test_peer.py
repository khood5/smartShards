...
class TestSawtoothMethods(unittest.TestCase):
    ...
    def test_peer_setup(self):
        ...
        self.assertEqual(p.id_a, id_a)
        self.assertEqual(p.id_b, id_b)
    ...
    def test_peer_join(self):
        ...
        for p in peers:
            ...
            for ip in committee_ips_B:
                if ip != p.ip(id_b):  # the peer it's self is not reported in the list
                    self.assertIn("tcp://{}:8800".format(ip), peers_config)
    ...
    def test_committee_independent_join(self):
        ...
        id_a = peers[0].id_a
        id_b = peers[0].id_b
        new_peer = Peer(SawtoothContainer(), SawtoothContainer(), id_a, None)
