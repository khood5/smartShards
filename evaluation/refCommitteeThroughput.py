...
def run_experiments(number_of_transactions: int, experiment_duration_secs: int, 
                    ...
                    experiments_end: int, committees: int):
    
    for experiment_number in range(experiments_start, experiments_end):

        print(f"Setting up experiment {experiment_number} with Committee number {committees} and Intersection of {number_of_intersections}")
...
def run_experiment(peers: dict, experiment_duration_secs: int, 
    ...
    for round in range(experiment_duration_secs):
        url_txs_by_round.append(create_txs(peers, committee_ids, number_of_transactions, peers_by_quorum, round))
...
def create_txs(peers, committee_ids, number_of_transactions, peers_by_quorum, round):
...
if __name__ == '__main__':
    run_experiments(NUMBER_OF_TX, EXPERIMENT_DURATION_SECS, INTERSECTION, EXPERIMENT_RANGE_START, EXPERIMENT_RANGE_END,NUMBER_OF_COMMITTEES)
