...
def get_avg_for(size: int, experiments: int, total_tx: int):
    ...
    for e in range(experiments):
        ...
        # peers =  make smart shard committees
        peers = None
...
def run_experiment(peers: list, total_tx: int):
    ...
    for i in range(total_tx):
        # tx = make tx with random committee id
        tx = None
        # submit tx to peer in system
        ...
        while not propagated:
            ...
            propagated = True
            for p in peers:
                if p.get_tx(tx) == '999' and not propagated:
                    waiting_time.append(time.time() - start_propagated)
